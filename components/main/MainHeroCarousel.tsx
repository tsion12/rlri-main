...
export function MainHeroCarousel() {
  ...
  useEffect(() => {
    if (slideCount <= 1 || reduceMotion || paused) return;
            ...
            {MAIN_HERO_SLIDES.map((slide, index) => {
              ...
              return (
                <button
                  key={slide.id}
                  type="button"
                  className="group relative flex h-11 w-11 items-center justify-center rounded-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/60"
                ...
                >
                  ...
                  {isActive && !reduceMotion && !paused && (
                    <span
                      className="pointer-events-none absolute -bottom-1 left-1/2 h-0.5 w-8 -translate-x-1/2 overflow-hidden rounded-full bg-white/25"
                      aria-hidden
                    >
                      <span
